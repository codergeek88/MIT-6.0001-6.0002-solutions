...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for w in range(self.width):
            for h in range(self.height):
                tile = (w, h)
                self.tile_dict[tile] = self.dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.tile_dict[tile] - capacity < 0:
            self.tile_dict[tile] = 0
        else:
            self.tile_dict[tile] -= capacity
...
#test_room = RectangularRoom(3, 5, 8)
#print(test_room.is_position_in_room(Position(3.1, 4.7)))
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        pos_x, pos_y = random.uniform(0, self.width), random.uniform(0, self.height)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        pos, angle = self.get_robot_position(), self.get_robot_direction()
        new_pos = pos.get_new_position(angle, self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    tiles_to_clean = math.floor(min_coverage * width * height)
    
    def createRoomandRobotList(robot_type, num_robots, width, height, dirt_amount, speed, capacity):
        room = EmptyRoom(width, height, dirt_amount)
        robotList = []
        for n in range(num_robots):
            robot = robot_type(room, speed, capacity)
            robotList.append(robot)
        return (room, robotList)
    
    def indivSim(robot_type, num_robots, width, height, dirt_amount, speed, capacity, tiles_to_clean):
        room, robotList = createRoomandRobotList(robot_type, num_robots, width, height, 
                                                 dirt_amount, speed, capacity)
        num_time_steps = 0
        while room.get_num_cleaned_tiles() < tiles_to_clean:
            for robot in robotList:
                robot.update_position_and_clean()
            num_time_steps += 1
        return num_time_steps
    
    def totalSim(robot_type, num_robots, width, height, dirt_amount, speed, capacity, 
                 tiles_to_clean, num_trials):
        time_steps_total = 0
        for n in range(num_trials):
            indiv_time_steps = indivSim(robot_type, num_robots, width, height, dirt_amount, 
                                        speed, capacity, tiles_to_clean)
            time_steps_total += indiv_time_steps
        time_steps_avg = math.floor(time_steps_total/num_trials)
        return time_steps_avg
    
    return totalSim(robot_type, num_robots, width, height, dirt_amount, speed, capacity, tiles_to_clean, num_trials)
